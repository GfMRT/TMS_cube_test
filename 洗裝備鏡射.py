...
equip_A_STR = [0.0444, 0.0364, 0.0444, 0.0408, 0.0444, 0.0488, 0.0606]
equip_A_all = [0.0444, 0.0364, 0.0444, 0.0408, 0.0444, 0.0488, 0.0606]
equip_S_STR = [0.0476, 0.05, 0.0588, 0.0488, 0.0541, 0.0588, 0.0784]
equip_S_all = [0.0476, 0.05, 0.0588, 0.0488, 0.0541, 0.0588, 0.0588]
equip_critical = 0.0976
equip_CD1 = 0.0714
equip_CD2 = 0.0476
skill_head_door_A = 0.0889
invincible_clothes_A = 0.0727
skill_pant_fire_A = 0.0889
skill_glove_eye_A = 0.0816
skill_shoe_speed_A = 0.0889
skill_head_bless_S = 0.0714
invincible_clothes_S = 0.075
skill_glove_speed_S = 0.0732
skill_shoe_order_S = 0.0811
item_get_S = 0.075

# 累進機率
for i in range(0,7):
    equip_A_all[i] = equip_A_all[i] + equip_A_STR[i]
...
def getA(state_percent, kind, only_appear): 
    line = random.random()
    
    if kind == 0: #帽子的實用
        if only_appear[0]==1:
            line = line * (1-skill_head_door_A)
        elif line>(1-skill_head_door_A):
            only_appear[0] = only_appear[0]+1
    elif kind == 1: #衣服的無敵
        if only_appear[0]==1:
            line = line * (1-invincible_clothes_A)
        elif line>(1-invincible_clothes_A):
            only_appear[0] = only_appear[0]+1
    elif kind == 2: #褲子的實用
        if only_appear[0]==1:
            line = line * (1-skill_pant_fire_A)
        elif line>(1-skill_pant_fire_A):
            only_appear[0] = only_appear[0]+1
    elif kind == 3: #手套的實用
        if only_appear[0]==1:
            line = line * (1-skill_glove_eye_A)
        elif line>(1-skill_glove_eye_A):
            only_appear[0] = only_appear[0]+1
    elif kind == 4: #鞋子的實用
        if only_appear[0]==1:
            line = line * (1-skill_shoe_speed_A)
        elif line>(1-skill_shoe_speed_A):
            only_appear[0] = only_appear[0]+1
...
for equip_i in range(0,7):
    ...
    for exp in range(0,int(1e7)):
        ...
        if level>0.8:
            ...
            getA(STR_percent, equip_i, only_appear)
        ...
        if equip_i == 0:
            if CD[1] >= 3:
                ...
                if CD[0] >= 1:
                    ...
                    CD4head_9 = CD4head_9+1
                elif max(STR_percent)==6.1:
                    CD4head_6all = CD4head_6all+1
                else:
                    CD4head = CD4head+1
    ...
    if equip_i == 0: #帽子
        print("    6CD: ", CD6head)
        print("    5CD: ", CD5head)
        print("    4CD: ", CD4head_12+CD4head_9all+CD4head_9+CD4head_6all+CD4head)
        print("        4CD13屬: ", CD4head_12)
        print("        4CD10全: ", CD4head_9all)
        print("        4CD10屬: ", CD4head_9)
        print("        4CD7全: ", CD4head_6all)
        print("        4CD爛: ", CD4head)
        print("    頂: ", top)
        print("    次頂: ", second_all+second)
        print("        含全: ", second_all)
        print("    次次: ", third_2top + third + third_1all + third_2all)
        print("        含6全: ", third_2top)
        print("        含9全: ", third_1all)
        print("        含18全: ", third_2all)
    elif equip_i == 3: #手套
        print("    三爆: ", top)
        print("    雙爆: ", second + second_all + third_2top + third_1all + third)
        print("        雙爆13: ", second)
        print("        雙爆10全: ", second_all)
        print("        雙爆10: ", third_2top)
        print("        雙爆7全:  ", third_1all)
        print("        雙爆爛: ", third)
    else:
        print("    頂: ", top)
        print("    次頂: ", second_all+second)
